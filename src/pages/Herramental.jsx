...
const EMPTY = { nombre: '', tipo: 'Disco', codigo: '', lote: '', sector: 'Corte' }
...
export default function Herramental() {
  ...
  function abrirEditar(h) { setForm({ nombre: h.nombre || '', tipo: h.tipo || 'Disco', codigo: h.codigo || '', lote: h.lote || '', sector: h.sector || '' }); setEditId(h.id); setModalOpen(true) }

  async function guardar() {
    if (!form.nombre.trim()) return toast.error('Ingresá el nombre')
    const payload = { nombre: form.nombre.trim(), tipo: form.tipo || null, codigo: form.codigo.trim() || null, lote: form.lote.trim() || null, sector: form.sector.trim() || null }
    setGuardando(true)
    const { error } = editId
            ...
            return (
              <div key={h.id} style={{ background: 'var(--surface)', border: `1px solid ${isDel ? 'rgba(255,85,119,0.4)' : 'var(--border)'}`, borderRadius: 'var(--radius-lg)', padding: '14px 16px' }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text3)', display: 'flex', flexDirection: 'column', gap: 2 }}>
                  ...
                  {h.sector && <div>Sector: {h.sector}</div>}
                </div>
                {!readOnly && (
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', width: '100%', maxWidth: 460 }}>
            ...
            <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div><label style={lbl}>Sector</label><input value={form.sector} onChange={e => setForm(f => ({ ...f, sector: e.target.value }))} placeholder="Ej: Corte" style={iSt} /></div>
