...
export default function PedidosCanal() {
  ...
  function abrirNueva() {
    setEditando(null)
    setFNroOrden(''); setFNombre(''); setFEmail(''); setFTel('')
    setFItems([emptyItem()]); setFObs(''); setFEstado('pendiente')
    setModal(true)
  }

  function abrirEditar(v) {
    setEditando(v)
    setFNroOrden(v.nro_orden || '')
    setFNombre(v.cliente_nombre || '')
    setFEmail(v.cliente_email || '')
    setFTel(v.cliente_telefono || '')
    setFItems(v.items?.length ? v.items.map(i => ({ ...i })) : [emptyItem()])
    setFObs(v.observaciones || '')
    setFEstado(v.estado || 'pendiente')
    setModal(true)
  }

  function calcTotal() {
    return fItems.reduce((s, it) => s + (parseFloat(it.precio_unitario) || 0) * (parseInt(it.cantidad) || 0), 0)
  }
  ...
  async function guardar() {
    ...
    const payload = {
      ...
      cliente_nombre: fNombre.trim(),
      cliente_email: fEmail.trim() || null,
      cliente_telefono: fTel.trim() || null,
      items: itemsValidos,
      total: calcTotal(),
      estado: fEstado,
    ...
    <div style={{ animation: 'fadeUp 0.35s ease' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 800 }}>
            {cc.emoji} Pedidos {cc.label}
          </h1>
          <p style={{ color: 'var(--text3)', marginTop: 4, fontSize: 13 }}>
            Gestión de ventas del canal {cc.label}
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10, marginBottom: 20 }}>
        {Object.entries(ESTADO_CONFIG).map(([k, ecfg]) => {
          ...
          return (
            <div key={k} onClick={() => setFiltro(active ? 'todos' : k)} style={{ background: 'var(--surface)', border: `1px solid ${active ? ecfg.border : 'var(--border)'}`, borderRadius: 'var(--radius-lg)', padding: '12px 16px', cursor: 'pointer', background: active ? ecfg.bg : 'var(--surface)' }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: ecfg.color }}>{count}</div>
              <div style={{ fontSize: 11, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.4px' }}>{ecfg.label}</div>
            </div>
          )
        })}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        <input value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="🔍 Buscar por cliente o N° de orden..." style={{ ...inputSt, maxWidth: 340 }} />
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
          {['todos', ...Object.keys(ESTADO_CONFIG)].map(e => {
            ...
            return (
              <button key={e} onClick={() => setFiltro(e)} style={{ padding: '5px 13px', borderRadius: 20, fontSize: 12, cursor: 'pointer', fontFamily: 'var(--font)', border: `1px solid ${active ? (ecfg.border || 'var(--border)') : 'var(--border)'}`, background: active ? ecfg.bg : 'transparent', color: active ? ecfg.color : 'var(--text3)', fontWeight: active ? 700 : 400 }}>
                {ecfg.label}
              </button>
            )
          })}
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: 60, color: 'var(--text3)', fontSize: 14 }}>
          No hay ventas{filtroEstado !== 'todos' ? ` con estado "${ESTADO_CONFIG[filtroEstado]?.label}"` : ''}
        </div>
      ) : (
          ...
          {filtradas.map(v => {
            ...
            return (
              <div key={v.id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', padding: '16px 20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 10, marginBottom: 10 }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
                      ...
                      {v.nro_orden && (
                        <span style={{ fontSize: 11, fontWeight: 700, color: cc.color, background: cc.bg, border: `1px solid ${cc.border}`, padding: '1px 8px', borderRadius: 10 }}>{v.nro_orden}</span>
                      )}
                ...
                {v.observaciones && (
                  <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 10 }}>💬 {v.observaciones}</div>
                )}
                ...
                <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', alignItems: 'center', borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                  {Object.entries(ESTADO_CONFIG).filter(([k]) => k !== v.estado).map(([k, ecf]) => (
                    <button key={k} onClick={() => cambiarEstado(v.id, k)} style={{ fontSize: 11, padding: '4px 12px', borderRadius: 20, cursor: 'pointer', fontFamily: 'var(--font)', background: ecf.bg, color: ecf.color, border: `1px solid ${ecf.border}`, fontWeight: 600 }}>
                      → {ecf.label}
                    </button>
                  ))}
                  <div style={{ flex: 1 }} />
          ...
          <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-lg)', width: '100%', maxWidth: 640, maxHeight: '92vh', overflowY: 'auto' }}>
            ...
            <div style={{ padding: '18px 22px 14px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, background: 'var(--surface)', zIndex: 1 }}>
              <div style={{ fontSize: 16, fontWeight: 700 }}>
                {editando ? '✏️ Editar venta' : `${cc.emoji} Nueva venta — ${cc.label}`}
              </div>
            ...
            <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  ...
                  <input value={fNroOrden} onChange={e => setFNroOrden(e.target.value)}
                    placeholder={canal === 'meli' ? 'Ej: 2000-1234567890' : 'Ej: ORD-00123'}
                    style={inputSt} />
                </div>
                <div>
              ...
              <div>
                ...
                <div style={{ marginTop: 8, textAlign: 'right', fontSize: 15, fontWeight: 800, color: cc.color }}>
                  Total: {formatPrecio(calcTotal())}
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={fObs} onChange={e => setFObs(e.target.value)} rows={2} placeholder="Notas internas, dirección de envío, etc." style={{ ...inputSt, resize: 'vertical', lineHeight: 1.5 }} />
              </div>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={() => setModal(false)} style={{ background: 'var(--surface2)', color: 'var(--text3)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '10px 16px', fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' }}>
                  Cancelar
                </button>
              </div>
            </div>
