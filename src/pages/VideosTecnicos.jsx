...
const CATALOG = [
  {
    ...
    subs: [
      { value: 'paneles_diagnostico',   label: 'Diagnóstico' },
      { value: 'paneles_reparacion',    label: 'Reparación' },
      { value: 'paneles_procedimiento', label: 'Procedimientos' },
    ],
  },
  {
    value: 'calefones',
    ...
    subs: [
      { value: 'calefones_diagnostico',   label: 'Diagnóstico' },
      { value: 'calefones_reparacion',    label: 'Reparación' },
      { value: 'calefones_procedimiento', label: 'Procedimientos' },
    ],
  },
  {
    value: 'anafes',
    ...
    subs: [
      { value: 'anafes_diagnostico',   label: 'Diagnóstico' },
      { value: 'anafes_reparacion',    label: 'Reparación' },
      { value: 'anafes_procedimiento', label: 'Procedimientos' },
    ],
  },
  {
    value: 'general',
    ...
    emoji: '🔧',
    color: '#b39dfa',
    bg: 'rgba(179,157,250,0.12)',
    border: 'rgba(179,157,250,0.35)',
    subs: [
      { value: 'general_herramientas',  label: 'Herramientas' },
      { value: 'general_procedimiento', label: 'Procedimientos Generales' },
    ],
  },
]
...
const EMPTY_FORM = { title: '', description: '', url: '', duration: '', category: 'paneles_diagnostico', download_url: '' }

export default function VideosTecnicos() {
  const { isAdmin, isTechService } = useAuth()
  ...
  async function load() {
    setLoading(true)
    let q = supabase.from('videos_tecnicos').select('*').order('created_at', { ascending: false })
    if (activeSub) {
  ...
  async function submit() {
    ...
    setSubmitting(true)
    if (editing) {
      const { error } = await supabase.from('videos_tecnicos').update({ ...form }).eq('id', editing.id)
      if (error) { toast.error('Error al actualizar'); setSubmitting(false); return }
      toast.success('Video actualizado ✅')
    } else {
      const { error } = await supabase.from('videos_tecnicos').insert({ ...form })
  ...
  async function deleteVideo() {
    const { error } = await supabase.from('videos_tecnicos').delete().eq('id', confirmDelete.id)
  ...
  return (
    <div style={{ animation: 'fadeUp 0.35s ease' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 28, fontWeight: 800 }}>Videos Técnicos</h1>
          <p style={{ color: 'var(--text3)', marginTop: 4, fontSize: 13 }}>Diagnóstico, reparación y procedimientos para técnicos de servicio</p>
        </div>
      ...
      {loading ? (
        ...
        <Empty icon="🔧" title="No hay videos en esta categoría" description={isAdmin ? 'Agregá el primer video desde el botón de arriba' : 'Todavía no hay videos cargados en esta categoría'} />
      ) : (
              ...
              >
                ...
                <div style={{ aspectRatio: s ? '9/16' : '16/9', maxHeight: s ? 280 : undefined, position: 'relative', overflow: 'hidden', background: '#000' }}>
                  ...
                  <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.28)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'rgba(74,108,247,0.92)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {/* Add / Edit Modal */}
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setEditing(null) }} title={editing ? '✏️ Editar Video Técnico' : '🔧 Agregar Video Técnico'}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <Input label="URL de YouTube" placeholder="https://www.youtube.com/watch?v=..." value={form.url} onChange={e => setForm(p => ({ ...p, url: e.target.value }))} />
          ...
          <Input label="Título" placeholder="Ej: Diagnóstico falla en resistencia Slim 500w" value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} />
          <Input label="Duración (opcional)" placeholder="Ej: 08:45" value={form.duration} onChange={e => setForm(p => ({ ...p, duration: e.target.value }))} />
          ...
          <Textarea label="Descripción (opcional)" placeholder="Describí brevemente el contenido del video..." value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={3} />
          <Input label="Link de descarga (opcional)" placeholder="https://... (manual PDF, esquema, etc.)" value={form.download_url} onChange={e => setForm(p => ({ ...p, download_url: e.target.value }))} />
