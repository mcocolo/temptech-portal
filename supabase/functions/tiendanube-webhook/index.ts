...
// Etiqueta corta para mostrar en logística (coincide con PRODUCTOS_LOG)
...
serve(async (req) => {
  // Tiendanube hace un GET para verificar el endpoint
  if (req.method === 'GET') {
  ...
  try {
    ...
    // Obtener detalle del pedido desde la API de Tiendanube
    ...
    const productos = (order.products || [])
      .map((p: any) => {
        ...
        return { codigo, label, cantidad: p.quantity ?? 1 }
      })
    ...
    // Calcular orden (al final de la lista del día)
    ...
    await supabase.from('logistica_diaria').insert({
      fecha,
      tipo: 'entrega_pt',
      nombre: `${customer.name ?? ''} — TN #${order.number}`,
    ...
    console.log(`✅ Logística creada: TN #${order.number} — ${customer.name}`)
